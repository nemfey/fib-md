...
#write.table(df, file = "credscoClean.csv", sep = ";", na = "NA", dec = ".", row.names = FALSE, col.names = TRUE)
...
#Hemos cambiado las variables a mano 1 a 1.

##############################################################################
#                                    Profiling 
#                           Package FactoMineR will be used
##############################################################################
dnum <- c(3,4,5,7,8,11,13,14,16)

library(FactoMineR)
summary(df$SBA_Appv)
# The "variable to describe cannot have NA ###################################
res.condes<-condes(df,16)
summary(df)
...
summary(df$State)
# The "variable to describe cannot have NA ###################################
res.catdes<-catdes(df,1, proba = 0.50)
